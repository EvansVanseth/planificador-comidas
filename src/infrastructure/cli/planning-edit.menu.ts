...
async function editarDatosPlanificacion(container: IContainer, planningId: string) {
  try {
...
async function gestionarCompra(container: IContainer, userId: string, planningId: string) {
  let continuar = true;
  while (continuar) {
    const opcion = await prompts({
      type: 'select',
