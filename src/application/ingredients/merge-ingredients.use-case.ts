...
export class MergeIngredientsUseCase {
  ...
  execute(userId: string, sourceIngredientId: string, targetIngredientId: string): void {
    if (sourceIngredientId === targetIngredientId) {
      throw new AppError('No se puede fusionar un ingrediente consigo mismo');
    }

    const source = this.ingredientRepository.findById(sourceIngredientId);
    if (!source) throw new AppError(`Ingrediente origen no encontrado: ${sourceIngredientId}`);

    const target = this.ingredientRepository.findById(targetIngredientId);
    ...
    const allRecipes = this.recipeRepository.findAllByUserId(userId);
    ...
    this.ingredientRepository.delete(sourceIngredientId);
  }
}
