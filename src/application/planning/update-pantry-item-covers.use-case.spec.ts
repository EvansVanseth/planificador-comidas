...
describe('UpdatePantryItemCoversUseCase', () => {
  ...
  it('debe actualizar las cubiertas de un item de la despensa', async () => {
    await useCase.execute(planningId, ingredientId, 3);
  ...
  it('debe fallar si el planning no existe', async () => {
    await expect(useCase.execute('inexistente', ingredientId, 3)).rejects.toThrow(AppError);
  });
});
