...
export class SetMealExclusionsUseCase {
  ...
  async execute(planningId: string, orderDay: number, momentTagId: string, exclusions: string[]): Promise<void> {
    ...
    await this.validarSinMomentoDia(exclusions);
  ...
  private async validarSinMomentoDia(tagIds: string[]): Promise<void> {
    for (const tagId of tagIds) {
      ...
      if (tag && tag.getDimension() === TagDimension.MOMENTO_DIA) {
        throw new AppError(`No se puede usar la etiqueta de momento del día "${tag.getName()}" como exclusión`);
      }
    }
  }
}
